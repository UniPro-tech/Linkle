...
export async function POST(request: NextRequest) {
  ...
  const res = await fetch(`${postScriptUrl as string}/club.php?filename=${clubId}_${fileName}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function DELETE(request: NextRequest) {
  const fileName = await request.nextUrl.searchParams.get("filename");
  const clubId = await request.nextUrl.searchParams.get("clubId");
  const res = await fetch(`${deleteScriptUrl as string}?type=club&filename=${clubId}_${fileName}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
